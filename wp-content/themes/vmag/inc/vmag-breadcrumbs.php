...
function vmag_breadcrumbs(){
  /* === OPTIONS === */
	...
	if (is_home() || is_front_page()) {
		if ($showOnHome == 1) echo '<div id="vmag-breadcrumbs"><a href="' . $homeLink . '">' . $text['home'] . '</a></div>';
	} else {
		echo '<div id="vmag-breadcrumbs" xmlns:v="http://rdf.data-vocabulary.org/#">' . sprintf($link, $homeLink, $text['home']) . $delimiter;
		...
		if ( get_query_var('paged') ) {
			...
			echo __( 'Page', 'vmag' ) . ' ' . get_query_var('paged');
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
		echo '</div>';
	}
