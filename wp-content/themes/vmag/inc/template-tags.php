...
function vmag_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( ' %s', 'post date', 'vmag' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( '%s', 'post author', 'vmag' ),
...
if( ! function_exists( 'vmag_post_tag_lists_hook' ) ) {
	function vmag_post_tag_lists_hook() {
		$post_tags_list = get_the_tag_list( '', esc_html__( '', 'vmag' ) );
		$post_tag_option = get_theme_mod( 'vmag_homepage_tag_option', 'show' );
		if ( $post_tags_list && $post_tag_option != 'hide' ) {
			printf( '<span class="post-tags-links">' . esc_html__( '%1$s', 'vmag' ) . '</span>', $post_tags_list ); // WPCS: XSS OK.
		}
	}
}
	...
	function vmag_post_comments() {
		...
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-count">';
				comments_popup_link( __( '0', 'vmag' ), __( '1', 'vmag' ), __( '%', 'vmag' ) );
			echo '</span>';
		}
	...
	function vmag_post_cat_lists() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && vmag_categorized_blog() ) {
				printf( '<span class="cat-links">' . esc_html__( '%1$s', 'vmag' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
		}
	}
endif;
...
if( ! function_exists( 'vmag_single_post_tags_list' ) ) :
	function vmag_single_post_tags_list() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( '', 'vmag' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links clearfix">' . esc_html__( '%1$s', 'vmag' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
	}
endif;